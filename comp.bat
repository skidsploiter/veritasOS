...
:: ==== Tools ====
set NASM=nasm              
:: the almighty assembler, turning ASM into ones and zeros
set QEMU=qemu-system-i386  
:: retro time machine, boot your stuff like it's 1995
...
:: bootloader but it's binary now
set KERNELBIN=kernel.bin       
:: kernel.exe but ancient
set IMAGE=floppy.img           
:: because we like pain and nostalgia
set PADDING=pad.bin            
:: pure zeroes, like my attention span

:: ==== Image size ====
set FLOPPY_SIZE=1474560        
:: 1.44MB of raw 90s power
...
:: if NASM cries, we die

:: ==== Assemble kernel ====
echo [*] Assembling kernel...
%NASM% -f bin %KERNEL% -o %KERNELBIN%
if errorlevel 1 goto error     
:: same deal, don't mess it up
...
:: slap it into QEMU, pray for BIOS
